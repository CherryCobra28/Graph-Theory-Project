...
\begin{document}
    ...
    \section{Literature Review}
        ...
        \subsubsection{Adjacency Matrix}
            Graphs can also be represented by adjacency matrices. We define the matrix $A$, entry $A_{i,j} = 1$ if there is an edge between node $i$ and node $j$ else $A_{i,j} = 0$. For an undirected graph $A$ is symmetrical. This can useful for quick computation that does not require any recursion.
        ...
        \subsection{Scale-Free Graphs}
            ...
            For $\gamma = 2$ the degree of the largest hub $k_{\text{max}}$ grows linearly with network size $N$. The average shortest path length does not depend on $N$. This is known as an Anomalous regime\\
        ...
        \subsection{Barabási-Albert model}
            ...
            \begin{align*}
                &\prod(k_i)=\frac{k_i}{\sum_{\forall j}k_j}\\
                &\text{Where $k_h$ is the degree of node $h$}
            \end{align*}
            Using these probabilities we add $m$ edges from our new unconnected node to $m$ unique nodes. We continue this $N-G_N$ times thus we will add $m(N-G_{N})$ edges
            \subsubsection{The Idea of Seed Graphs}
            ...
            \subsubsection{Degree Distribution}
            The Barabási-Albert model produces scale free graphs. The degree distribution of it is: $p_{k} \sim k^{-3}$ \parencite{barabasi2013network}. As we can see from the section on scale free graphs the Barabási-Albert model exists in the $2<\gamma<3$ range. Note that the degree distribution is independent of $N$ and the number of iterations the model takes.
    ...
    \section{APPENDIX}
        ...
        \includegraphics[width=5cm]{images/add_nodes_add_edges.png}\\
